...
//Coge las coordenadas y saca los m2 totales del área dibujada
  area: function(coordss) {
  console.log("Coordss de qSVG");
  console.log(coordss);
  ...
  polygonize: function(segment) {
    ...
    for (var jc = 0; jc < edgesChild.length; jc++) {
        ...
        if (WAYS.length > 0) {
          ...
          var bestArea = parseInt(lengthRoom);
            ...
            if (vertex[bestVertex].bypass == 0)  { // <-------- TO REVISE IMPORTANT !!!!!!!! bestArea Control ???
              ...
              //var realArea = qSVG.area(realCoords.outside);
      var realArea = bestArea; //Mi area real, es bestarea (originalmente estaba como inside).
      var outsideArea = qSVG.area(realCoords.outside);
              ...
              // WARNING -> FAKE
      //CAMBIO EL AREA POR EL VALOR DE BESTAREA PARA QUE ME CALCULE LOS M2 CORRECTAMENTE
      //SI NO, SI UTILIZA realArea, tiene en cuenta los m2 INTERNOS y eso no es lo que queremos
              if (realCoords.inside.length != realCoords.outside) {
                polygons.push({way: tempSurface, coords: coords, coordsOutside: realCoords.outside, coordsInside: realCoords.inside, area: bestArea, outsideArea: outsideArea, realArea: bestArea});
              }
              else { // REAL INSIDE POLYGONE -> ROOM
                polygons.push({way: tempSurface, coords: realCoords.inside, coordsOutside: realCoords.outside, area: bestArea, outsideArea: outsideArea, realArea: bestArea});
              }

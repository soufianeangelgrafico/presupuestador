...
<head>
	...
	#menu_plano, #menu_mobiliario, #menu_instalaciones {
     width: 33%;
     display: inline-block;
     padding: 25px;
		min-height:120px;
...
?>	 
 <div id="parte_superior">
	...
	<div id="menu_planificador">
		<div id="menu_plano" class="menu-activo"><h2 class="titular-planificador"><span class="paso-menu">1.</span> <b style="color:black">PASO 1:</b> CREAR PLANO</h2></div>
		<div id="menu_mobiliario"><h2 class="titular-planificador"><span class="paso-menu">2.</span> <b style="color:black">PASO 2:</b> MOBILIARIO</h2></div>
		<div id="menu_instalaciones">
			<h2 class="titular-planificador"><span class="paso-menu">3.</span><b style="color:black">PASO 3:</b> INSTALACIONES Y <span class="secondline">EQUIPAMIENTO</span></h2>
	...
	<div id="panel_informacion">
	 ...
	 <div id="opciones_crear_plano">
		  <div id="panel">
			...
			<p>  
			  <button class="btn fully door" id="aperture">Añadir puerta</button>
			  <button class="btn fully window" id="fix">Añadir ventana</button>
			</p>
		  </div>
			...
			<div id="informacion_pared">
				...
				<select name="altura_paredes" id="altura_paredes" class="form-control">
				 ...
				 <option value="2.35">2.35 m</option>
				 <option value="2.40">2.40 m</option>
				 <option value="2.45">2.45 m</option>
				 <option value="2.50">2.50 m</option>
				 <option value="2.55">2.55 m</option>
				 <option value="2.60">2.60 m</option>
				 <option value="2.65">2.65 m</option>
				 <option value="2.70">2.70 m</option>
				 <option value="2.75">2.75 m</option>
				 <option value="2.80">2.80 m</option>
				 <option value="2.85">2.85 m</option>
				 <option value="2.90">2.90 m</option>
				 <option value="2.95">2.95 m</option>
				...
				if (isset($_GET["id_presupuesto"]))
				{
					...
					else
					{
					  ...
					  while ($arr_result_paredes_totales = $result_paredes_totales->fetch_array())
					   	 $paredes_totales=$arr_result_paredes_totales["paredes_totales"];
		...
		?>		
				<li>
					...
					?>
						  ...
						  <div class="listado_muros" <?php if (isset($_GET["id_presupuesto"])) {?> style="display:block;" <?php } ?></div>
		...
		<?php		
		 ...
		 ?>		
				<li>
					<?php echo nl2br($arr_result['nombre']);?>
					<br>
					<img src="<?php echo $arr_result['imagen'];?>" class="object elemento_instalacion <?php if (isset($_GET["id_presupuesto"]) && $elemento_seleccionado) {?> selected <?php } ?>" id="<?php echo $arr_result['id_imagen'];?>">
					<?php
					  if ($arr_result["mostrar"] == "muros")
					  {
						  //Le dejo que seleccione el/los muros a los que va esta acción
...
<script>
  $( document ).ready(function() {
	...
	$(".elemento_instalacion").click(function() {
		 ...
		 //input de unidades
		if ($("#"+this.id+" + .unidades_elemento").length)
		 {
			if (selected == 1)
	 ...
	 $("#btnFinalizar").click(function() {
		 ...
		 $(".elemento_instalacion.selected").each(function() {
			 ...
			 $.ajax({
				 ...
				 dataType: 'json', 
				 async:false,
				 url: "ajax/finalizar_plano.php?total_muros="+total_muros,
				 data: formulario,
		  ...
		  console.log("FORMULARIOS!!");
			...
			// var contenido=localStorage.getItem("history");
			 ...
			 $.ajax({
					 ...
					 success:function(respuesta){	
						 	window.location.assign("https://rehubik.com/presupuestador/2d/elegir_materiales.php"); 
					 }
	 ...
	 $("#btnFinalizarVerificador").click(function() {
		 ...
		 $(".elemento_instalacion.selected").each(function() {
			 ...
			 $.ajax({
				 ...
				 dataType: 'json', 
				 async:false,
				 url: "ajax/finalizar_plano_verificador.php?id_presupuesto=<?php echo $_GET['id_presupuesto'];?>",
				 data: formulario,
				 success:function(respuesta){	
					 alertify.success(respuesta.mensaje);
	...
	$("select[name='unidades']").change(function() { 
		...
		$("path[stroke-dasharray='"+clase+"']").parent().remove();	//Elimino el padre de esos elementos, para eliminar el icono

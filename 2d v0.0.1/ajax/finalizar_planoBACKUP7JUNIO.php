		...
		while ($arr_result_material = $result_material->fetch_array())
		{
			...
			if ($mostrar == "muros")
			{
				...
				for ($i=0;$i<count($paredes);$i++)
				{
					...
					$mysqli->query("INSERT sesiones_temporales(referencia_material,sesion,paredes,longitud_pared,altura_pared,total_paredes) VALUES('$elemento','$sesion_temporal','$pared_actual','$longitud_pared','$altura_paredes','$total_paredes')");
			...
			else
			{
				...
				$mysqli->query("INSERT sesiones_temporales(referencia_material,sesion,altura_pared,total_paredes) VALUES('$elemento','$sesion_temporal','$altura_paredes','$total_paredes')");
...
else
{
	...
	if ($mostrar == "muros")
	{
		...
		for ($i=0;$i<count($paredes);$i++)
		{
			...
			*/
			$mysqli->query("INSERT sesiones_temporales(id_articulo_compuesto,sesion,paredes,longitud_pared,altura_pared,total_paredes) VALUES($id_articulo_compuesto,'$sesion_temporal','$pared_actual','$longitud_pared','$altura_paredes','$total_paredes')");
			//echo "INSERT sesiones_temporales(id_articulo_compuesto,sesion,paredes,longitud_pared,total_paredes) VALUES($id_articulo_compuesto,'$sesion_temporal','$pared_actual','$longitud_pared','$total_paredes')";
		}
	...
	else if ($mostrar == "metros_lineales")
	{
		...
		$mysqli->query("INSERT sesiones_temporales(id_articulo_compuesto,sesion,total_paredes,metros_lineales) VALUES($id_articulo_compuesto,'$sesion_temporal','$total_paredes','$metros_lineales')");	
	...
	else
	{
	 ...
	 /* $precio_final=$unidades*$precio;
	  $suma_precios=$suma_precios+$precio_final;	
		*/ 
	  $mysqli->query("INSERT sesiones_temporales(id_articulo_compuesto,sesion,total_paredes,unidades) VALUES($id_articulo_compuesto,'$sesion_temporal','$total_paredes',$unidades)");	
	}

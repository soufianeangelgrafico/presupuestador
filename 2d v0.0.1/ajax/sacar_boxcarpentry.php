...
$result=$mysqli->query("SELECT dibujo_puertas_ventanas_reformado FROM clientes WHERE id_presupuesto=$id_presupuesto AND dibujo_puertas_ventanas_reformado != ''");
//echo "SELECT valor FROM clientes WHERE id_presupuesto=$id_presupuesto AND valor != ''";
while ($arr_result = $result->fetch_array())
{
	$respuesta->mensaje=$arr_result["dibujo_puertas_ventanas_reformado"];
}

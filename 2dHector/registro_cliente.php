	  ...
	  <div class="col-md-12">
	   ...
	   <?php
		  ...
		  else
		  {
			  ...
			  $result_altura_pared=$mysqli->query("SELECT altura_techo_reformado FROM sesiones_temporales WHERE sesion='$sesion_temporal' AND altura_techo_reformado != 0 LIMIT 1");
			  
			  $result_alto_ancho=$mysqli->query("SELECT metros_ancho_reformado,metros_alto_reformado FROM sesiones_temporales WHERE sesion='$sesion_temporal' AND metros_ancho_reformado != 0 AND metros_alto_reformado != 0");
			  
			  while ($arr_result_altura_pared = $result_altura_pared->fetch_array())
			    $altura_pared=$arr_result_altura_pared["altura_techo_reformado"];
			  
			 while ($arr_result_alto_ancho = $result_alto_ancho->fetch_array())
			 {
				 
				 $metros_ancho_reformado=$arr_result_alto_ancho["metros_ancho_reformado"];
				 $metros_alto_reformado=$arr_result_alto_ancho["metros_alto_reformado"];
				 
			 }
			  
			  
			  //$result_material=$mysqli->query("SELECT sesiones_temporales.referencia_material as referencia_material, sesiones_temporales.paredes_reformado as paredes,sesiones_temporales.longitud_pared_reformado as longitud_pared, sesiones_temporales.altura_pared_reformado as altura_pared,sesiones_temporales.total_paredes as total_paredes,materiales.rvpv as rvpv, materiales.modelo as modelo, materiales.pvp as precio,materiales.mostrar as mostrar FROM sesiones_temporales,materiales WHERE materiales.referencia = sesiones_temporales.referencia_material AND sesion='$sesion_temporal' AND referencia_material IS NOT NULL");
				   ...
				   ?>	
						<tr>
						  ...
						  <th scope="row" colspan='2'>
							...
							<td><?php if ($paredes != "") { ?> <?php echo $paredes;?><br>(<?php echo $longitud_pared;?>m seleccionados) <?php } ?></td>
						   <td>
							   <?php 
							    if ($rvpv == "Pavimento")
								{
								   ...
								   $precio_final=round($area * $altura_pared * 1.15,2)*$precio_material;
								  
									
								}
								else
								{
								  $precio_final=round($longitud_pared * $altura_pared * $precio_material * 1.15,2)*$precio_material;
								}

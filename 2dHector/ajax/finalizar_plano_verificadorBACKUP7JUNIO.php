		...
		while ($arr_result_material = $result_material->fetch_array())
		{
		  ...
		  if ($arr_result_material["mostrar"] == "muros")
		  {
			...
			for ($i=0;$i<count($paredes);$i++)
			{
				...
				$mysqli->query("INSERT clientes(id_usuario,referencia_material,paredes,longitud_pared,altura_pared,total_paredes,id_presupuesto_modificado) VALUES($id_usuario,'$elemento','$pared_actual','$longitud_pared','$altura_paredes','$total_paredes',$id_presupuesto)");
...
else
{
	...
	else if ($mostrar == "metros_lineales")
	{
	    ...
	    $suma_precios=$suma_precios+$precio_final;
		 ...
		 for ($x=0;$x<count($articulo_simple);$x++)
		  ...
		  $id_articulo_simple_actual=$articulo_simple[$x];
		  $mysqli->query("INSERT clientes(id_usuario,id_articulo_compuesto,id_articulo_simple,total_paredes,metros_lineales,id_presupuesto_modificado) VALUES($id_usuario,$id_articulo_compuesto,$id_articulo_simple_actual,'$total_paredes','$metros_lineales',$id_presupuesto)");
	...
	else
	{
	  ...
	  for ($x=0;$x<count($articulo_simple);$x++)
	  {
		 ...
		 $id_articulo_simple_actual=$articulo_simple[$x];
		 $mysqli->query("INSERT clientes(id_usuario,id_articulo_compuesto,id_articulo_simple,total_paredes,unidades,id_presupuesto_modificado) VALUES($id_usuario,$id_articulo_compuesto,$id_articulo_simple_actual,'$total_paredes','$unidades',$id_presupuesto)");
